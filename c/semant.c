...
Ty_ty tylook(S_table tenv, S_symbol type_name, int pos) {
  ...
  if (nameTy) {
    ...
    EM_error(pos, "undefined type %s", S_name(type_name));
    return NULL;
  }
}
...
void transDec(S_table venv, S_table tenv, A_dec d, Tr_level level) {
  switch (d->kind) {
    case A_varDec: {
      ...
      Tr_access access = Tr_allocLocal(level); 
    ...
    case A_functionDec: {
      ...
      Tr_level new_level = Tr_newLevel(level, name);
      ...
      S_beginScope(venv);
      {
        ...
        for (l = f->params, t = formalTys; l; l = l->tail, t = t->tail) {
          Tr_access access = Tr_allocLocal(new_level); 
          S_enter(venv, l->head->name, E_VarEntry(access,t->head));
        }
      }
      transExp(venv, tenv, d->u.function->body, new_level);
      S_endScope(venv);
      break;
    }
  }
...
bool detect_cycles(S_table tenv, S_symbol name) {
  ...
  if (tortoise->kind != Ty_name) return 0;
  tortoise = tortoise->u.name.ty;
  Ty_ty hare = S_look(tenv, name);
  if (hare->kind != Ty_name) return 0;
  hare = hare->u.name.ty;
  if (hare->kind != Ty_name) return 0;
  hare = hare->u.name.ty;
  while (1) {
...
struct expty transExp(S_table venv, S_table tenv, A_exp a, Tr_level level) {
  switch (a->kind) {
      ...
      for (d = a->u.let.decs; d; d = d->tail) {
        if (d->head->kind == A_typeDec) {
          if (detect_cycles(tenv, d->head->u.type->name)) {
            EM_error(a->pos, "Cycle in definition of %s",
                     S_name(d->head->u.type->name));
          }
        }
      }
      A_expList p;
      for (p = a->u.let.body; p != NULL; p = p->tail) {
        exp = transExp(venv, tenv, p->head, level);
      }
    ...
    case A_opExp: {
      ...
      if (oper == A_neqOp || oper == A_ltOp || oper == A_gtOp ||
          oper == A_geOp || oper == A_andOp || oper == A_orOp) {
        if (left.ty->kind != right.ty->kind)
          EM_error(a->pos, "Operand types must match");
      }
      if (oper == A_eqOp) {
        if (left.ty->kind != right.ty->kind && left.ty->kind != Ty_nil &&
    ...
    case A_seqExp: {
      ...
      if (seq != NULL) {
        struct expty head = transExp(venv, tenv, seq->head, level);
        while (seq->tail != NULL) {
          seq = seq->tail;
          head = transExp(venv, tenv, seq->head, level);
        }
        return head;
      } else {
    ...
    case A_recordExp: {
      ...
      while (fields != NULL) {
        ...
        struct expty headType = transExp(venv, tenv, head->exp, level);
    ...
    case A_arrayExp: {
      ...
      if (ty && ty->kind == Ty_array) {
        ...
        EM_error(a->pos, "Unknown array type: %s", S_name(typ));
      }
    }
    case A_callExp: {
    ...
    case A_ifExp: {
      ...
      struct expty test_result = transExp(venv, tenv, test, level);
      if (test_result.ty->kind != Ty_int) {
        EM_error(test->pos, "Test condition must have integer type");
      }

      struct expty then_result = transExp(venv, tenv, then, level);

      if (elsee) {
        struct expty elsee_result = transExp(venv, tenv, elsee, level);
    ...
    case A_whileExp: {
      ...
      struct expty body_result = transExp(venv, tenv, body, level);
      return body_result;
    }
    case A_forExp: {
      S_beginScope(venv);
      ...
      struct expty lo_result = transExp(venv, tenv, lo, level);
      struct expty hi_result = transExp(venv, tenv, hi, level);
...
void SEM_transProg(A_exp exp) {
  ...
  add_std_types(tenv);
  transExp(venv, tenv, exp, Tr_outermost());
}
